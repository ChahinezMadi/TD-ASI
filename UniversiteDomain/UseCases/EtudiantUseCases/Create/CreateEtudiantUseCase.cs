...
public class CreateEtudiantUseCase(IRepositoryFactory repositoryFactory)
{
    ...
    public async Task<Etudiant> ExecuteAsync(Etudiant etudiant)
    {
        ...
        repositoryFactory.SaveChangesAsync().Wait(); // même style que ton code d'origine
    ...
    private async Task CheckBusinessRules(Etudiant etudiant)
    {
        ...
        if (existe is { Count: > 0 })
            throw new DuplicateNumEtudException(
                etudiant.NumEtud + " - ce numéro d'étudiant est déjà affecté à un étudiant");
        ...
        if (existe is { Count: > 0 })
            throw new DuplicateEmailException(
                etudiant.Email + " est déjà affecté à un étudiant");
